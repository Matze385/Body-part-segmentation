...
"""
predict hard segmentation fast image out of given rawdata with index img with given clf_list
"""
def predict_hard_one_image_new_data(img, rawdata_filename, clf, n_classes, sigmasGaussian, sigmasLoG=[],sigmasGGM=[],sigmasSTE=[],sigmasHoGE=[] ,sigmasGaussian_prob=[], sigmasLoG_prob=[],sigmasGGM_prob=[],sigmasSTE_prob=[],sigmasHoGE_prob=[]):
#returns y2D_pred array of shape [xDim,yDim] of type np.int32
#img: index for frame that is predicted
    ...
    for c in np.arange(cDim):
        for i,sigma in enumerate(sigmas):
            if sigma in sigmasGaussian:
                sel_feat[:,:,sigmasGaussian.index(sigma)+c*n_feat_per_class] = vg.filters.gaussianSmoothing(rawdata[:,:,c], sigma)
            offset=len(sigmasGaussian)  
            if sigma in sigmasLoG:
                sel_feat[:,:,sigmasLoG.index(sigma)+c*n_feat_per_class+offset] = vg.filters.laplacianOfGaussian(rawdata[:,:,c], sigma)
            offset=len(sigmasGaussian)+len(sigmasLoG)  
            if sigma in sigmasGGM:
                sel_feat[:,:,sigmasGGM.index(sigma)+c*n_feat_per_class+offset] = vg.filters.gaussianGradientMagnitude(rawdata[:,:,c], sigma)
            offset=len(sigmasGaussian)+len(sigmasLoG)+len(sigmasGGM)
            if sigma in sigmasSTE:
                data1 = vg.filters.structureTensorEigenvalues(rawdata[:,:,c], sigma, sigma )
                sel_feat[:,:,2*sigmasSTE.index(sigma)+c*n_feat_per_class+offset] = data1[:,:,0]
                sel_feat[:,:,2*sigmasSTE.index(sigma)+1+c*n_feat_per_class+offset] = data1[:,:,1]
            offset=len(sigmasGaussian)+len(sigmasLoG)+len(sigmasGGM)+len(sigmasSTE)*2
            if sigma in sigmasHoGE:
                data2 = vg.filters.hessianOfGaussianEigenvalues(rawdata[:,:,c], sigma)
                sel_feat[:,:,2*sigmasHoGE.index(sigma)+c*n_feat_per_class+offset] = data2[:,:,0]
                sel_feat[:,:,2*sigmasHoGE.index(sigma)+1+c*n_feat_per_class+offset] = data2[:,:,1]

    
    n_stages = len(clf)
    #determine number of features out of prob maps 
    n_feat_per_class_prob = len(sigmasGaussian_prob)+len(sigmasLoG_prob)+len(sigmasGGM_prob)+2*len(sigmasSTE_prob)+2*len(sigmasHoGE_prob)
    n_add_features = n_classes*n_feat_per_class_prob
    #allocate arrays
    y2D_pred = np.zeros((xDim, yDim), dtype=np.int32)            
    X_pred = sel_feat.reshape((xDim*yDim, sel_feat.shape[2]))
    if n_stages==1:
        y2D_pred = clf[0].predict(X_pred).reshape((xDim, yDim))
        return y2D_pred
    #feature array for features out of rawdata and prob maps
    all_features = np.zeros((xDim,yDim, sel_feat.shape[2]+n_add_features), dtype=np.float32)
    all_features[:,:,:sel_feat.shape[2]] = sel_feat[:,:,:]
    for i_stage in np.arange(n_stages-1):
        i_stage +=1
        if i_stage-1==0:
            prob1D = clf[0].predict_proba(X_pred)
            prob_map = prob1D.reshape((xDim, yDim, n_classes))
            all_features[:,:,sel_feat.shape[2]:] = calc_selected_std_features_one_img(prob_map, sigmasGaussian_prob, sigmasLoG_prob,sigmasGGM_prob, sigmasSTE_prob,sigmasHoGE_prob)     
            #if last classifier
            if i_stage==n_stages-1:
                X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
                y2D_pred = clf[i_stage].predict(X_pred_all).reshape((xDim, yDim))     
                return y2D_pred        
        else:
            X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
...
#clf: list of clf for different stages
    ...
    n_add_features = n_classes*n_feat_per_class_prob
    #allocate arrays
    y2D_pred_hard = np.zeros((xDim, yDim), dtype=np.int32)            
    y2D_pred_prob = np.zeros((xDim, yDim, n_classes), dtype=np.int32)            
    X_pred = sel_feat.reshape((xDim*yDim, sel_feat.shape[2]))
    if n_stages==1:
        y2D_pred_hard = clf[0].predict(X_pred).reshape((xDim, yDim))
        y2D_pred_soft = clf[0].predict_proba(X_pred).reshape((xDim, yDim, n_classes))
        return y2D_pred_soft, y2D_pred_hard
    #feature array for features out of rawdata and prob maps
    all_features = np.zeros((xDim,yDim, sel_feat.shape[2]+n_add_features), dtype=np.float32)
    all_features[:,:,:sel_feat.shape[2]] = sel_feat[:,:,:]
    for i_stage in np.arange(n_stages-1):
        ...
        if i_stage-1==0:
            ...
            if i_stage==n_stages-1:
                X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
                y2D_pred_hard = clf[i_stage].predict(X_pred_all).reshape((xDim, yDim))     
                y2D_pred_soft = clf[i_stage].predict_proba(X_pred_all).reshape((xDim, yDim, n_classes))     
                return y2D_pred_soft, y2D_pred_hard        
        else:
            X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
            prob1D = clf[i_stage-1].predict_proba(X_pred_all)
...
"""
predict prob maps of one image out of given rawdata with index img with given clf_list
"""
def predict_prob_one_image_new_data(img, rawdata_filename, clf, n_classes, sigmasGaussian, sigmasLoG=[],sigmasGGM=[],sigmasSTE=[],sigmasHoGE=[] ,sigmasGaussian_prob=[], sigmasLoG_prob=[],sigmasGGM_prob=[],sigmasSTE_prob=[],sigmasHoGE_prob=[]):
#returns y2D_pred array of shape [xDim,yDim, n_classes] of type np.int32
#img: index for frame that is predicted
#rawdata_filename: name of hdf5 file for prediction, format [t,x,y,c]
#clf: list of clf for different stages
    ...
    n_add_features = n_classes*n_feat_per_class_prob
    #allocate arrays
    y2D_pred = np.zeros((xDim, yDim, n_classes), dtype=np.int32)            
    X_pred = sel_feat.reshape((xDim*yDim, sel_feat.shape[2]))
    if n_stages==1:
        y2D_pred = clf[0].predict_proba(X_pred).reshape((xDim, yDim, n_classes))
        return y2D_pred
    #feature array for features out of rawdata and prob maps
    all_features = np.zeros((xDim,yDim, sel_feat.shape[2]+n_add_features), dtype=np.float32)
    all_features[:,:,:sel_feat.shape[2]] = sel_feat[:,:,:]
    for i_stage in np.arange(n_stages-1):
        ...
        if i_stage-1==0:
            ...
            if i_stage==n_stages-1:
                X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
                y2D_pred = clf[i_stage].predict_proba(X_pred_all).reshape((xDim, yDim, n_classes))     
                return y2D_pred        
        else:
            X_pred_all = all_features.reshape((xDim*yDim, sel_feat.shape[2]+n_add_features))
            prob1D = clf[i_stage-1].predict_proba(X_pred_all)
